...
/// nome di remote rclone può iniziare per `/`). Stessa funzione duplicata
/// in `mounts.rs`/`jobs.rs`/`bisync.rs`: qui serve una copia propria per
/// non introdurre una dipendenza incrociata tra quei tre moduli.
