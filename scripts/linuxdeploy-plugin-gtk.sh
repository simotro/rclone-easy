...
# Copia locale di linuxdeploy-plugin-gtk.sh (allineata al 2026-09-22 su
# https://raw.githubusercontent.com/tauri-apps/linuxdeploy-plugin-gtk/master/linuxdeploy-plugin-gtk.sh),
# con una sola differenza rispetto all'originale: l'hook scritto in
# apprun-hooks/ non esporta piu` GDK_BACKEND=x11. tauri-bundler scaricherebbe
# l'originale solo se non trova gia` un file con questo nome in
# ~/.cache/tauri/ (vedi lo step "Prepara l'hook GTK per l'AppImage" nei
# workflow CI, che ce lo mette prima della build) — se questo file diverge
# troppo dall'originale in futuro (nuove versioni di GTK, nuova logica di
# bundling), va risincronizzato a mano da quell'URL.
#
